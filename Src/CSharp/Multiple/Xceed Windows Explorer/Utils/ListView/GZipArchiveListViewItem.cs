...
{
  public class GZipArchiveListViewItem : FileListViewItem
  {
    #region CONSTRUCTORS

    public GZipArchiveListViewItem( AbstractFile file, AbstractTreeViewNode parentNode )
      : this( file, parentNode, string.Empty )
    {
    }

    public GZipArchiveListViewItem( AbstractFile file, AbstractTreeViewNode parentNode, string displayText )
    ...
    public override void Create()
    {
      ...
      AbstractTreeViewNode node = new GZipArchiveTreeViewNode( this.File, this.ListView );
    ...
    public override void Delete( bool confirmDelete )
    {
      ...
      if( confirmDelete )
      {
        ...
        string caption = "Confirm GZip Archive Delete";

        if( itemIsReadonly )
          message = string.Format( "The GZip archive '{0}' is a read-only file. Are you sure you want to delete it?", m_item.Name );
    ...
    public override void Rename( string name )
    {
      ...
      string originalName = this.File.Name;
    ...
    protected override void FillData()
    {
      ...
      // Type
      string type = typeof( Xceed.GZip.GZipArchive ).ToString();
      this.SubItems.Add( type );

      // Modification date
      string modifiedDateFormatted = "N/A";
      
      if( this.File.HasLastWriteDateTime )
      {
        modifiedDateFormatted = this.File.LastWriteDateTime.ToString( 
